...
- [x] Erlang Port-based live transport (spawn `mlld live --stdio`)
- [x] GenServer client with automatic transport restart
- [x] `process/2`, `execute/3`, `analyze/2` synchronous APIs
- [x] `process_async/2`, `execute_async/3` returning Task-compatible handles
- [x] `cancel/1`, `update_state/3` on handles
- [x] All types: ExecuteResult, AnalyzeResult, StateWrite, Effect, Metrics, Error
- [x] Request multiplexing with integer IDs
- [x] Event dispatch (state:write routing)
- [x] Per-request timeout with cancel
- [x] Transport death detection and pending request failure
- [x] Module-level convenience functions
- [x] Integration test suite matching other SDKs
- [ ] Hex package published as `mlld`
- [x] README with examples

### Phase 2: Elixir-Native Features
- [x] Supervision tree integration (child_spec, start_link)
- [x] Connection pool (Mlld.Pool) with checkout/checkin
- [x] :telemetry event emission
- [x] Phoenix Channel bridge module (optional dependency)
- [x] Named registration and discovery
...
**2026-02-18 18:21 UTC:** Implemented phase 1/2 code in sdk/elixir (GenServer client, Port transport, protocol/types, async handles, cancel/update_state, pooling, telemetry, optional Phoenix bridge) plus detailed README and parity integration tests. Remaining unchecked phase-1 item: Hex publish. Local execution validation blocked in this environment because elixir/mix binaries are not installed.

**2026-02-18 19:03 UTC:** Elixir now available: ran sdk/elixir test suite successfully (7 tests, 0 failures). Added sdk/elixir/RELEASE.md with full Hex release workflow and linked release section in sdk/elixir/README.md.
