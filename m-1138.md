...
**2026-02-12 20:45 UTC:** m-3b36 (let shadows var) is now confirmed fixed — let inside blocks throws VariableRedefinition when redefining outer var-scoped variables. Since the leading hypothesis was that stale state from let shadowing caused this crash, m-1138 may be resolved.

To verify: run the J2BD security pipeline (mlld run j2bd/security) through at least 2 full decision loop iterations with a worker that commits. The crash occurred after friction_reported was logged in iteration 2+. If the pipeline completes without '2 condition errors', close this ticket.

**2026-02-12 20:55 UTC:** Not a design decision — verification task. m-3b36 fix may have resolved this. Needs a j2bd pipeline run to verify. If it doesn't reproduce, close.

**2026-02-13 18:12 UTC:** Presumptively closed — many changes since this was filed (including m-3b36 let-shadows-var fix). If it resurfaces, reopen.
